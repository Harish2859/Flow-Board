...
export function TaskDetailModal({ task, members, canEdit, onClose, onDeleted, onUpdated }: Props) {
  ...
  const setPrio = (p: Priority) => {
    if (!canEdit) return;
    setPriority(p);
    update({ priority: p });
  };

  const setAssign = (uid: string | null) => {
    if (!canEdit) return;
    setAssigned(uid);
  ...
  return (
    <div
      className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-start justify-center p-4 sm:items-center overflow-y-auto"
      onClick={onClose}
    >
      <div
        className="w-full max-w-xl rounded-xl border border-border bg-card p-6 my-8"
      ...
      >
          ...
          <div className="mt-1.5 flex gap-2">
            {(["low", "medium", "high"] as Priority[]).map((p) => (
              <button
                ...
                disabled={!canEdit}
                onClick={() => setPrio(p)}
                className={`flex-1 inline-flex items-center justify-center gap-2 rounded-md border px-3 py-2 text-xs font-medium capitalize transition ${
                  priority === p
        ...
        <div className="mt-5">
          ...
          <select
            disabled={!canEdit}
            value={assigned ?? ""}
            onChange={(e) => setAssign(e.target.value || null)}
